...
export default {
    ...
    methods:{
        onclick(index){ //点击分类
            ...
            this.page = 1;
            if(this.categorTitle == '全部'){
                this._getData_Imgs(this.page)
            }else{
                let obj = {
                    page:this.page,
        ...
        onPreview(index){//当前所有预览数据
            ...
            if(this.categorTitle == '全部'){
                     this.$api.getData_Resources(0,this.pageSize,'imgs').then(res=>{
                    ...
                    this.$api.getData_ImgsSearch(obj).then(res=>{
                        //  console.log(res.data);
                         this.previewData = [...res.data.data]
                         this.$refs.prv.setsetCurrentIndex(index)
                    })
        ...
        _getData_ClassImgs(){   //请求图片分类数据
                this.$api.getData_ClassImgs('相片').then(res=>{
                    ...
                    this.categorTitle = '全部'
                    this.isLoad = true
                    this._getData_Imgs(this.page)
                })
        },
        _getData_ImgsSearch(data){  //获取分类详情数据
